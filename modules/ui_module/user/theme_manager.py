...
class ThemeManager(QObject):
    ...
    BRAND_YELLOW = "#d5b618"
    BRAND_BLUE   = "#345ddb" 
    LIGHT_GREY = "#e6e6e6"  # used for replacing white/default backgrounds in dark mode
    DARK_BG = "#26272b"     # dark theme background color
    ...
    _QSS_DARK = f"""
        * {{ font-family: "Microsoft YaHei UI", "Segoe UI", "Noto Sans TC"; }}
        ...
        QTabBar::tab {{ background:#1f2023; color:#b5b8bf; padding:10px 18px; margin-right:4px;
                        border-top-left-radius:8px; border-top-right-radius:8px; border:1px solid #2f3136; font-weight:600; }}
        QTabBar::tab:selected {{ background:#26272b; color:#ffffff; border:1px solid {BRAND_YELLOW}; }}
        QTabBar::tab:hover:!selected {{ background:#232427; }}

        QGroupBox#settingsGroup {{ background:#1f2023; border:1px solid #2f3136; border-radius:10px;
                                   margin-top:12px; padding-top:18px; color:#e6e6e6; font-weight:600; }}
        ...
        QComboBox, QLineEdit, QSpinBox, QPlainTextEdit, QTextEdit {{
            background:#26272b; color:#e6e6e6; border:1px solid #2f3136; border-radius:8px; padding:8px 12px;
            selection-background-color:{BRAND_YELLOW}; selection-color:#000000; }}
        QComboBox:focus, QLineEdit:focus, QSpinBox:focus, QPlainTextEdit:focus, QTextEdit:focus {{
            border:1px solid {BRAND_YELLOW}; }}

        QPushButton {{ background:{BRAND_YELLOW}; color:#000000; border:none; border-radius:10px; padding:10px 18px; font-weight:700; }}
        QPushButton:hover {{ background:#e6c51c; }}
        QPushButton:pressed {{ background:#b89f14; }}
        QPushButton#themeToggle{{ background:#000; color:#e6e6e6; border:none; min-width:56px; min-height:56px; border-radius:28px; font-size:20px; padding:0; }}
        QPushButton#headerClose{{ background:#1f2023; color:#e6e6e6; border:1px solid #2f3136; min-width:56px; min-height:56px; border-radius:28px; font-size:18px; padding:0; }}
        ...
        QCheckBox::indicator:checked, QRadioButton::indicator:checked {{ background:{BRAND_YELLOW}; border-color:{BRAND_YELLOW}; }}

        QSlider::groove:horizontal {{ background:#1f2023; height:8px; border-radius:4px; }}
        QSlider::handle:horizontal {{ background:{BRAND_YELLOW}; width:18px; height:18px; border-radius:9px; margin:-6px 0; }}
        ...
        QTreeWidget::item:selected {{ background:{BRAND_YELLOW}; color:#000000; }}

        /* Replace default/white backgrounds in certain widgets with dark theme background */
        ...
        QListWidget,
        QTextEdit,
        QPlainTextEdit {{ background:{DARK_BG}; border:1px solid #2f3136; border-radius:8px; }}
        ...
        QPlainTextEdit > QWidget#qt_scrollarea_viewport {{ background:{DARK_BG}; }}
        QWidget#qt_scrollarea_vcontainer,
        QWidget#qt_scrollarea_hcontainer {{ background:{DARK_BG}; }}

        /* All scrollbars - dark background */
        QScrollBar {{ background:#3a3b40; border:none; }}
...
def install_theme_hook(widget):
    ...
    try:
        ...
        if s:
            ...
            cleaned = s
            # When in dark theme, replace explicit white backgrounds with a light grey
            try:
                if theme_manager and theme_manager.theme == Theme.DARK:
                    ...
                    # For light theme, keep existing stylesheet untouched (or strip legacy explicit white markers)
                    cleaned = cleaned.replace("background: #ffffff;", "")
                    cleaned = cleaned.replace("background:#ffffff;", "")
                    cleaned = cleaned.replace("background-color: #ffffff;", "")
                    cleaned = cleaned.replace("background-color:#ffffff;", "")
            except Exception:
                pass
