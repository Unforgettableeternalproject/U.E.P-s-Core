...
try:
    from PyQt5.QtWidgets import (
        ...
        QFormLayout, QGridLayout, QApplication, QMessageBox, QListWidget, QStatusBar, QDialog
    )
...
class UserSettingDialog(StateProfileDialog):
    def __init__(self, parent=None, controller = None):
        if controller is not None and parent is None:
            super().__init__(controller=controller, parent=None)
        else:
...
class UEPStateProfileWidget(QWidget):
    ...
    def _card_text_label(self, mono_hint: bool = False) -> QLabel:
        lb = QLabel()
        ...
        f.setPointSize(11)
        if mono_hint:
            f.setFamily("Consolas, Menlo, JetBrains Mono, monospace")
        lb.setFont(f)
        return lb
    ...
    def _refresh_image(self):
        ...
        if not path or not os.path.exists(path):
            demo = "/mnt/data/istockphoto-1542379335-612x612.jpg"
...
class StateProfileDialog(QMainWindow):
    settings_changed = pyqtSignal(str, object)
    ...
    def _build_ui(self):
        ...
        # Main content area with diary widget
    ...
    def _create_header(self, parent_layout: QVBoxLayout):
        ...
        self.theme_toggle.clicked.connect(self._on_toggle_theme)
    ...
    def _sync_theme_icon(self):
        if theme_manager:
            is_dark = (theme_manager.theme == Theme.DARK)
    ...
    def _handle_apply_request(self, data: dict):
        if hasattr(self, 'controller') and self.controller:
            try:
                ...
                print(f"[StateProfileDialog] Failed to communicate with controller: {e}")
